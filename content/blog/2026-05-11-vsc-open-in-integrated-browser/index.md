---
title: "AI时代，需求痒了就自己挠 —— 20分钟又搓了个 VSCode 插件"
...
在 VSCode 里预览 Markdown，直接点右上角的"预览"按钮就行，一秒的事。HTML？没有这个按钮。得自己开命令面板，找 `Browser: Open Integrated Browser`，再把文件路径复制粘进去。每次改完文件重新预览，万一不小心关闭了，还得再来一遍……（或者是我根本没有找到更好的办法..? 有了解的同学请指点）

![VSCode 命令面板中的 Browser: Open Integrated Browser 命令](img-01.jpg)
...
![右键文件直接"在内置浏览器中打开"](img-02.jpg)

## 有一点要说实话

如果你从没发布过 VSCode 插件，光是搞清楚发布流程（注册 Azure DevOps 账号、生成 PAT、用 `vsce` 打包上传）可能就要花不少时间——不难，但第一次总要摸索一遍。

我之前发过 [**CSM-vscode-support**](https://marketplace.visualstudio.com/items?itemName=NEVSTOP-LAB.csm-vsc-support)，流程熟，这次直接绕过了这段"学习成本"。所以你如果也想试试用 AI 快速做个插件，还需了解对应的背景知识，把发布这关打通，后面就是 AI 的体力活了。

## 插件信息

**🌐 Open in Integrated Browser**
[![Marketplace](https://img.shields.io/visual-studio-marketplace/v/NEVSTOP-LAB.vsc-open-in-integrated-browser?label=VS%20Marketplace)](https://marketplace.visualstudio.com/items?itemName=NEVSTOP-LAB.vsc-open-in-integrated-browser)
