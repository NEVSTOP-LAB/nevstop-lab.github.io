...
![CSM Modules 侧边栏入口](image.png)

进入视图后，每个模块以卡片形式呈现，整体体验类似于浏览扩展市场：

![CSM Modules 模块卡片列表](image-1.png)
...
除了浏览卡片，视图还提供了灵活的筛选和排序能力：

![CSM Modules 搜索与过滤](image-2.png)
...
- LabVIEW 版本检测：自动提取版本信息（如 `lv2020`、`lv2020(64bit)`）
- 中英文界面切换

功能清单比较密集，接下来通过一个具体的示例工程来展示这些能力在实际项目中如何协同工作。

## 实战示例：csm-community-example

[csm-community-example](https://github.com/NEVSTOP-LAB/csm-community-example) 是一个完整的示例工程，专门用于演示 CSM Modules 的端到端使用流程。

![CSM VSCode 插件侧边栏中同时展示 VSCode 工作区与 LabVIEW 项目](Snipaste_2026-06-03_21-21-50.png)
...
可以直接 clone [csm-community-example](https://github.com/NEVSTOP-LAB/csm-community-example)，安装 CSM VSCode 插件后即可体验完整的模块管理效果。
...
- **模块源有待扩展**：当前仅支持 GitHub
- **LabVIEW 依赖管理**：目前模块引入仅处理仓库本身，不会自动解析和安装模块所依赖的 LabVIEW 库（如 `.lvlib`、VIPM 包等）；模块的 LabVIEW 级依赖仍需在 README 中手动声明并由用户自行安装

如果对这一方向感兴趣，欢迎通过以下方式参与：

- 参与讨论未来的功能规划和设计，提交 [Issue](https://github.com/NEVSTOP-LAB/csm-vsc-extension/issues) 反馈问题或提出建议
